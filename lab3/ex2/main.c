...
int main(int argc, char * argv[]) {
   ...
   int residual = appconf.arrsz % appconf.tnum;
   struct _range* idx_range[appconf.tnum];
   for(i = 0 ; i<appconf.tnum;i++){
   		idx_range[i] = malloc(sizeof(struct _range));
   }
   idx_range[0]->start = 0;
   idx_range[0]->end = appconf.arrsz/ appconf.tnum -1 ;
   if(residual > 0 ){
   		idx_range[0]->end ++;
   		residual--;
   }
   for (i=1;i<appconf.tnum;i++){
   		idx_range[i]->start = idx_range[i-1]->end +1;
   		idx_range[i]->end = idx_range[i]->start + appconf.arrsz/ appconf.tnum -1;
   		   if(residual > 0 ){
   		idx_range[i]->end ++;
   		residual--;
   }
   ...
   for (i = 0 ;i<appconf.tnum;i++){
   		pthread_create(&tid[i], NULL, sum_worker, (void*) idx_range[i]);
   }
   for (i=0; i < appconf.tnum; i++)
      {
